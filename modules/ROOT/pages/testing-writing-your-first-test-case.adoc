...
[NOTE]
====
If your flow returns a stream, you must call the _keepStreamsOpen()_ method to prevent the runtime from closing streams (which is the normal behavior when a flow ends), otherwise when trying to consume a stream you'll get a _CursorProviderAlreadyClosedException_. As example:
