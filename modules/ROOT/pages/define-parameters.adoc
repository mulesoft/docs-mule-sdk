...
The Java classes that define your POJO parameters (regardless if they are used with the @ParameterGroup annotation) *must* implement the *equals* and
...
Even if no configurations have POJO parameters, it is still required that they implement *equals* and *hashCode* as future
runtime functionality may depend on it. Below is an example from a class in the Database Connector that illustrates how to do this:
[source,java,linenums]
----
/**
