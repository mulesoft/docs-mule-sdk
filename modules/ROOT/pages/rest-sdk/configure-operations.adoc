...
* using an HTTP `method` and HTTP full `path` with an `operationId` with a null value to execute a fallback
...
`operationId` with a non null value or a `method` and a `path` together.
...
The following example shows a default DataWeave script that relies on a
`method` and a `path` together:
...
resources, which is not useful to the user of the connector. For example, in
`/2010-04-01/Accounts/{id}/Messages/SMS/Media.json`, both `/2010-04-01/` and
`.json` are not useful to the user.

The following example shows a DataWeave script using `operationIdentifier` that
eliminates these parts:
...
The following example shows a DataWeave script using `method` and `path` that eliminates these parts from
...
The following example shows another DataWeave script using `method` and `path` that eliminate specific parts from the
