...
Use an operation identifier in REST SDK to generate classes for resources.
Fetch an operation identifier from either an OAS model
under `operationId` or a RAML model. If fetched from a RAML model, the operation
identifier defaults to null.

The identifier cannot collide with resources, triggers, or other REST
SDK components. REST SDK provides a default mechanism to generate an operation
identifier if there is a valid `operationId` from an OAS context with a non
null value. It also executes a fallback if there is an HTTP method and
...
The resource `/2010-04-01/Accounts/{id}/Messages/SMS/Media.json` is now
