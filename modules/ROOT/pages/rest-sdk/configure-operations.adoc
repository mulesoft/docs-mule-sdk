...
Configure and customize an existing API operation of the connector descriptor of the connector.
...
To change a generated display name for an operation, you must use a
DataWeave script.
...
comprehension rather than extension. The default DataWeave script relies on an
`operationId` (if it is not null) or a `method` and a `path` together.
...
For more information about the two functions in the default DataWeave script, refer to
...
The following example shows a DataWeave script that eliminates these parts from
...
It is not necessary to use `operationId` since it is null for this scenario.
...
The following example shows another DataWeave script that eliminate specific parts from the
