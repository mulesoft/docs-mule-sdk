...
Fetch an operation identifier from an OAS model under `operationId`. If fetched
from a RAML model, the operation identifier defaults to null.
...
unique resource name, and executes only if `operationId` is null.
