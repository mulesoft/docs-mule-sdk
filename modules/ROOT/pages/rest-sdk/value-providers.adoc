...
In this example, the `postMessage` method will have a Value Provider for the body field `channel` with possible values that can be obtained by making a request to another endpoint `/conversations.list`
...
You can then add this value provider to the channels field in the body as shown:
...
This example contains the previous parameter provider with more intelligence. This value provider uses other body field to query for the appropriate type of channels.
...
In this example, the `postMessage` method will have a Value Provider for the body field `channel` with possible values that can be obtained by making a request to another endpoint `/conversations.list` with a query parameter `channelType`.
...
In this example, the channelType query parameter is bound to the value provided in the body field `uri` inside the nested object `channelType`.
