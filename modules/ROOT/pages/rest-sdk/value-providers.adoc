= Customize the Connector's Value Providers
...
The following example shows a definition of an inline value provider:
...
Global definitions provide reuse capabilities. A globally declared value provider
enables defining parameters to build a context of where it can be executed.

The following example shows a definition of a global value provider:
