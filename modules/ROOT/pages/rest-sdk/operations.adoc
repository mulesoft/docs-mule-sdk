...
Configure and customize an existing operation of the connector descriptor of the connector.

== Set an Operation Identifier for an Operation

To set an operation identifier for an operation, use the REST SDK
...
To set a display name for an operation, use the `displayName` property,
...
To change a generated display name for an operation, use either the
...
If you are using an `operationId` in your custom DataWeave script, change
...
To set a description for an operation, use the `description` property,
which is located in the operation node.
...
To select a default media type for an operation, use the `inputMediaType`
...
defined in xref:rest-sdk/operations.adoc#select-default-media-type-for-an-operation[Select a Default Media Type for an Operation], but you want
to do it globally, use the `defaultInputMediaType` and `defaultOutputMediaType`
...
To ignore an operation or an endpoint, use the `ignored` property,
...
To ignore operations and triggers by default, use the `ignoreOperations`
...
To hide an operation, use the `ignore` property.
...
To rename an operation, use the `displayName` property.
...
To redefine an input type or an output type for an operation, use the
`inputType` and `outputType` properties, which are located at the operation node.
...
To override an input type or an output type for an operation, use the
...
To skip output type validations for an operation, use the `skipOutputTypeValidation`
...
To declare void for an operation, use the `voidOperation` property, which is located
in the operation node.
