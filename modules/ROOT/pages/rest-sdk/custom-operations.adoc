...
The JSON Schema for Location referenced by the spec above should look like this:
...
Add a display name (mandatory) and description to the custom operation:
...
The following snippet brings another supported alternative to define the base connectivity for the *Find Location* operation
...
Even though both options are supported, we strongly encourage the use of operationId when available, as it represents a more sustainable choice throughout the evolution and changes the API spec may face.
...
The sidecar directive generates all customized operations as
