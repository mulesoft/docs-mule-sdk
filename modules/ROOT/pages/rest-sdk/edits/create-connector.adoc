...
You can also add additional parameters to the `createConnector` mojo using
the `-D` prefix. In the example command above, the *apiSpec* parameter and the *descriptor* parameter are added using
the `-D` prefix since they are required parameters. The `createConnector` mojo uses
