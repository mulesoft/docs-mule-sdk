...
When assigning values, it is also possible to access fields from `body`. The following example shows how to access a field *channelId* that is assigned as a parameter for the resolver:
...
----
The value used to access fields inside the `body` uses DataWeave's accessors and is valid for the input metadata type defined for the `body` parameter.
