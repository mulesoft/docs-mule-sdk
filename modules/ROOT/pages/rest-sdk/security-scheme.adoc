...
To set a display name for a security scheme parameter, you must use the
...
To ignore a security scheme, you must use the `ignored` property, which is located
...
To add parameters for OAuth 2.0 authorization, you must define the parameters in
the security node.
...
To customize the OAuth refresh token condition, you must use the `refreshTokenCondition`
...
The following example shows a customized refresh token condition for
this scenario:
