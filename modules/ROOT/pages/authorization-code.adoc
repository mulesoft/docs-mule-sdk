...
<1> Notice that the connection is receiving the state and not the access token. Access tokens can be refreshed or unauthorized at any time leaving the connection useless, it is important to always query the state object any time the access token is needed. <<define-configurations-and-connection-providers#_always_query_the_state_object_on_each_request, Check for more info>>
