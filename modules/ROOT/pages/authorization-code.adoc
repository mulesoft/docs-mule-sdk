...
== Configure an Authorization Code Connector

OAuth-enabled connectors automatically support the Authorization Code grant type. This section describes the parameters and how to configure them.

=== Synthetic Parameters

In addition to the parameters defined in the `ConnectionProvider`, the SDK automatically injects some standard OAuth parameters for Authorization Code.
...
|SUPPORTED
|N.A.
|The URL of the authorization endpoint to start the OAuth dance.

|`accessTokenUrl`
|YES
|SUPPORTED
|N.A.
|The URL to request the access token after the OAuth dance.

|`scopes`
|NO
|SUPPORTED
|N.A.
|The OAuth scopes to request during authorization. If not provided, defaults to those in the annotation.
...
|SUPPORTED
|N.A.
|A flow to execute before authentication.

|`after`
|NO
|SUPPORTED
|N.A.
|A flow to execute after authentication.

|`authServerTlsContext`
|NO
|NOT_SUPPORTED
|N.A.
|A reference to the TLS context configuration used for the authorization server.

|`authServerProxyConfig`
|NO
|NOT_SUPPORTED
|N.A.
|A reference to the proxy configuration used for the authorization server.
|===

=== About the Use of Expressions

Many of the synthetic parameters accept expressions. Using expressions allows the configuration to be dynamic, just like with regular parameters.

=== OAuth Connection DSL

The following example shows a basic Authorization Code connection configuration.
...
This example shows how to configure a TLS context for the Authorization Code flow.
...
<test-oauth:auth-code-config name="oauth">
    <test-oauth:connection display="PAGE" immediate="true" prompt="false">
        <test-oauth:oauth-authorization-code consumerKey="${consumerKey}"
                                             ...
                                             scopes="${scopes}"
                                             after="afterFlow"
                                             authServerTlsContext="tlsContextServer" />
...
This example shows how to set a proxy for the Authorization Code flow.
...
<test-oauth:auth-code-config name="oauth">
    <test-oauth:connection display="PAGE" immediate="true" prompt="false">
        <test-oauth:oauth-authorization-code consumerKey="${consumerKey}"
                                             ...
                                             scopes="${scopes}"
                                             after="afterFlow"
                                             authServerProxyConfig="proxyConfig" />
    </test-oauth:connection>
</test-oauth:auth-code-config>
...
=== Custom ObjectStore Configuration

Access tokens are stored in an Object Store. By default, the SDK uses the app's default Object Store, but you can define a custom one.
...
<test-oauth:auth-code-config name="oauth">
    <test-oauth:connection display="PAGE" immediate="true" prompt="false">
        <test-oauth:oauth-authorization-code consumerKey="${consumerKey}"
                                             ...
                                             scopes="${scopes}"
                                             after="afterFlow" />
        <test-oauth:oauth-store-config objectStore="tokenStore"/>
    </test-oauth:connection>
</test-oauth:auth-code-config>
----
