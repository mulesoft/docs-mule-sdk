...
they produce. These examples can show information obtained from a live system
through a connection or show the output format from a connectionless example.

Sample data is available for Mule Runtime 4.4.0 or later, however connectors
can use it without upgrading their Minimum Mule Version.
...
that is representative of the data the component will output in real production
scenarios.
...
return a Result<T,A> object. Generic types must also match the return types of
the enhanced component. For components that return dynamic types, each
implementation must return instances of the correct dynamic type. If the
operation does not return attributes, use Result<T, Void>.
* Guarantee that fetching sample data will not generate any significant side
...
For connectors, implementations are the most valuable when returning real data
from the system they connect to instead of returning hard coded values.

The following is an example of what the SampleDataResolver API looks like:
...
* Operations or sources that return an InputStream. The resolver must return
implementations that do not require a connection, such as a ByteArrayInputStream
or any other offline implementations that have already procured the entire data
stream before returning the getSample() method, as the InputStream must be fully
...
cases, a List<T> of the same item type should be returned instead.

=== SampleDataException

This class communicates errors while fetching sample data so that the
runtime can handle them accordingly. You can use the predefined failure code
messages described in this class.

The following is an example of what the SampleDataException class looks like:
...
This annotation associates a resolver to the enhanced component. It can be
applied at the type level in sources and at the method level for operations.

The following is an example of what the @SampleData annotation looks like:
...
This example shows how to provide static sample data without using a connection.

Suppose there is a new GitHub connector that contains an operation that
...
number of calls or because it is too costly. Instead, you can directly implement
the operation.

The following is a definition of the operation with the provider attached
...
The following is the sample data provider implementation, which uses the same
...
Suppose the value for the *username* parameter is John Doe. The provider will
...
This example provides dynamic sample data according to parameter and
configuration values, while also requiring a connection.

Suppose there is a new GitHub connector that contains both an operation and
source that:

* Have an *objectType* parameter that controls the type of entity retrieved. It
has possible values such as pullRequest, issue, or commit.
* Have an optional *status* parameter that allows filtering the results.
* Use a connection to hit the GitHub API.
* Return a Result<InputStream, ObjectAttributes> object.

The following shows the operation definition:
...
The following shows the source definition:
...
The following shows the SampleDataProvider implementation that can serve both
cases. It not only uses the *objectType* and *status* parameters, but also uses
...
also define the same parameters that the resolver needs, but they both have
...
If your operation or source uses an OAuth connection, you can trigger a token
refresh in the provider by throwing an *AccessTokenExpiredException* inside
the getSample() method, like you would do for
https://docs.mulesoft.com/mule-sdk/1.1/oauth-token-expiration[operations or sources].

The following is an example of using a token refresh:
...
supported consistently, just like defining an acting parameter as part of a
parameter.
...
validation error will be thrown otherwise.
...
declarations the parameters are of the same type. A validation error will be
thrown otherwise.

=== Connection and Configuration Usage Check

The SDK validates that if a sample data provider uses a connection and/or
configuration, the related component also uses them. A validation error will
be thrown otherwise.

=== Instantiation Check

The SDK validates that each provider is instantiable, which is a required
condition. A validation error will be thrown otherwise.
