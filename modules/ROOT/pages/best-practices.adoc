...
Before publishing your module, try it in Studio and Flow Designer. Make sure that it provides a good usability and experience. A module that works but is hard to use is not complete.
...
* `.api.*` contains all the classes that are to be exported and seen by the Mule app: Return Types, Subtypes, Input Types, and Connection interfaces.
...
[TIP]
You should *ONLY* export packages defined in your module. *Never* export packages from other dependency libs.
...
== Transactions
Non-transactional operations should not receive a transactional connection. The SDK will make those operations transactional, and in Studio, your user will be wrongfully presented with transactional options.
