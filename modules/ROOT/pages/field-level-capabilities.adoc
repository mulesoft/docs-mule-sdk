...
There are several features for Java SDK that support field level capabilities
for value providers and sample data. Using these features enables you to define
value providers for fields of parameters or use fields of parameters as acting
parameters for either a value provider or sample data.

These features are available for Mule Runtime 4.4.0, however connectors can use
them without upgrading their Minimum Mule Version.

== Defining a Value Provider for a Part of a Parameter

You can declare a value provider for a field of a parameter by using the
...
* `targetSelectors`
+
Describes which part of a parameter holds values given by the value provider.
...
a parameter named *body*, which is a JSON InputStream:
...
*body*, which is a JSON InputStream that has two parts with values that
can be provided:
...
*channelId*, that have values that are independent from each other.
...
XML, you can use this syntax to define a value provider for an attribute that
...
In this example, the *body* parameter is an XML that has a root tag named
...
The following is an example of @SampleData annotation binding an acting
...
that is used to provide the sample data. The value of this acting parameter
...
will always be application/java. This means that the provider must not expect
...
parameter. For example, if the parameter is a JSON InputStream but the static
