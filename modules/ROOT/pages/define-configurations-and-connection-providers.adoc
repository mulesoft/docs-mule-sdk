...
=== Don't use the access token in the connection creation

The creation of the connection object in the ConnectionProvider#connect method must not depend on the presence of a valid access token. Even if the connection needs to use the token to perform a one-time only request, that should not be done in the creation of the connection. The request must be deferred to when the connection is first used.

To this end, the org.mule.runtime.api.util.LazyValue class can be leveraged. So that when a value is first used, the action of retrieving the value is done, and later uses will get the value that was cached from the first time.
...
Now, the first time the LazyValue is used, the information will be retrieved, and all later usages will get the already resolved value.
