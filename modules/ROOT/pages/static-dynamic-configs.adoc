...
Note that there is a difference between a parameter with a value that is not an expression and a parameter that does not accept expressions. The former might accept an expression as a value.
...
It is also possible to configure the global default at an app level so that any dynamic configuration has a different default. You set this up through the `<configuration>` element in the xref:mule-runtime::about-mule-configuration.adoc[Mule DSL], which is provided for you in Anypoint Studio:
