...
used when generating schemas. If empty, the module uses a hyphenated version
of the XML SDK `name` attribute.

Similarly, the `namespace` attribute in the `<module>` element is the namespace
of the module used when generating schemas. If empty, the module uses a
