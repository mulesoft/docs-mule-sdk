...
| required
|
| The type of error code to throw (or remap) in the `<body>`. More info about xref:mule-runtime::mule-error-concept.adoc[Mule Error concept].
...
The XML SDK relies on xref:mule-runtime::mule-error-concept.adoc#error-mappings[error mappings] for the former. For the latter, the xref:mule-runtime::raise-error-component-reference.adoc[raise error component] is used.
...
Use the `xmlns:tlsEnabled=true` annotation to indicate the component to support TLS. The annotated element does not necessarily have to be a top level element. In the following example, the annotated element is `<http:request-connection>` which is a child of the `<http:request-config>` top level element.
...
The `<tls:context>` parameter can be specified only in one component. Using it on more than one component results in an error. 
...
When the extension is used, the supplied `<tls:context>` parameter is mapped to the config element with this annotation. 

==== Inline Declaration

The following example shows an inline declaration of the `<tls:context>` parameter:
...
The following example shows a by-reference declaration of the `<tls:context>` parameter:
...
In this case, the value that comes from the module definition is used as the default. The synthetic parameter is still added to the module’s config, which, if provided, is used as an override. Providing a default value at the target element makes the synthetic parameter optional even if the parameter at the target element is required. This applies to both inline and by-reference declarations.
