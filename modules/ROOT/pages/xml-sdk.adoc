...
Otherwise, the project throws a build error, for example, `Attribute 'required-parameter' is not allowed to appear in element 'tns:shared-operation'.`

The XML definition of the connector generates parameters set with `use=CONTENT` as inner
tags. Refer to xref:content-parameters.adoc#content-param[this example] for File
Connector, in which the parameter named *content* is set with `use=CONTENT` as the
inner tag `<file:content>`.
