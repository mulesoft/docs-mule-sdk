...
| optional
| NA
| The prefix of the module used when generating schemas. If empty, a hyphenated version of `name` is used. Refer to <<prefix-attribute>>.
...
The `prefix` attribute in the `<module>` element is the prefix of the module
used when generating schemas. If it is empty, the module uses a hyphenated
version of the XML SDK `name` attribute.

Similarly, the `namespace` attribute in the `<module>` element is the namespace
of the module used when generating schemas. If it is empty, the module uses a
hyphenated version of the XML SDK `prefix` attribute.

Note that the value of the `prefix` attribute correlates with the name of the
