...
`ParameterResolver<Type>` allows you to defer the resolution of an expression on a Operation. This is useful when a parameter that takes an expression as a value is not required. You can improve performance by resolving such an expression on demand from within the connectors's code instead of resolving it automatically.
