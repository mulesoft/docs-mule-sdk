...
As indicated previously, there are 3 possible strategies: *offset*,
*marker* and *pageNumber*.
...
For *offset* pagination, the available parameters are:
...
For *marker* pagination, the available parameters are:
...
nextTokenExpression:: This is a dataweave expression that the connector
uses to extract the next page token from the API response. This token is
then used to request the page that follows.

For *pageNumber* pagination, the available parameters are:
...
The _pagingResponseExpression_ is common to all the pagination
...
*content-search-pagination*: _#Declares a pagination strategy to be used
within this API_
