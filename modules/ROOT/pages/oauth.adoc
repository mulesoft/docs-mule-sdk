...
OAuth is an authentication protocol used by many APIs to allow an app to act on behalf of a user without sharing the user's credentials. If you are unfamiliar with the protocol, see https://oauth.net/2/[https://oauth.net/2/].
...
a _very_ complex protocol) from the end user and provide a simpler, easier experience that does not force users to go through the learning curve of OAuth.
