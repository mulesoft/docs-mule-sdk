...
Be aware that setting the `@MetadataScope` annotation at the operation class definition level indicates that *all* of its operations provide metadata and thus they all must have a parameter annotated with `@MetadataKeyId`, while they inherit the resolver from the scope. However, if you only wish to set a key resolver for a single operation you can do so like this:
...
If there is a `@MetadataScope` annotation at the operation class definition level, setting a value for the `@MetadataKeyId` annotation in a parameter will override the resolver defined at the scope level.
[source,java,linenums]
----
