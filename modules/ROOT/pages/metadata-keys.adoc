...
MetadataKey parameters can be made optional like the other parameters by adding the ``@Optional` annotation. For example:
...
When an optional MetadataKey parameter does not have a value, metadata calls are made using null as MetadataKey. This means that any MetadataResolver associated with this MetadataKey must be prepared to receive a null value as MetadataKey.
