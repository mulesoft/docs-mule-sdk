...
After the resolver is defined, you can use it as an operation or source by adding a `@MetadataScope` annotation with its `keysResolver` value at the class definition level and also annotating the parameter that is the key with `@MetadataKeyId`. All metadata resolvers must be of the same category in an operation or source.
