...
Once the resolver is defined, you can use it an operation or source, by adding a `@MetadataScope` annotation with its `keysResolver` value at the class definition level, while also annotating the parameter that will be the key with `@MetadataKeyId`.
...
----
NOTE: For Sources, the method described above is the only way to support metadata keys. Do not set the resolver at the `@MetadataKeyId` annotation when defining sources.

Be aware that setting the `@MetadataScope` annotation at the operation class definition level (like the example given above) indicates that *all* of its operations provide metadata and thus they all must have a parameter annotated with `@MetadataKeyId`, while they inherit the resolver from the scope. However, if you only wish to set a key resolver for a single operation you can do so like this:
...
Another useful feature is that, if there is a `@MetadataScope` annotation at the operation class definition level, setting a value for the `@MetadataKeyId` annotation in a parameter will override the resolver defined at the scope level only for that operation:
[source,java,linenums]
----
...
IMPORTANT: The two features described above (setting the resolver at the parameter level and overriding the inherited resolver from the scope) only work when defining Operations, not for Sources.
...
In order to declare it, you simply annotate a parameter with `@MetadataKeyId`, but you do not provide a `TypesKeyResolver` neither for the parameter annotation nor for the `@MetadataScope` annotation should it be present at the class definition.
