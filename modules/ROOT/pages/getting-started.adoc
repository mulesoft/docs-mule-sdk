...
NOTE: The parent POM defines a Minimum Mule Version by default, that version must be compatible with the target Mule version. See xref:forward-compatibility.adoc[Forward Compatibility] for more information on how to configure and work with Minimum Mule Versions.
