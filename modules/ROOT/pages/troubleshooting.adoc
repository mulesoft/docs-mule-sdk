...
To troubleshoot Java SDK, learn how to enable logging and verbose logging, as well as interpret common error messages.
...
when using version 1.4.0 of the module's parent POM:
...
[IMPORTANT]
Use the annotations and classes from `org.mule.runtime.extension.api` unless
you need new functionality from `org.mule.sdk.api`. If you need new annotations
from `org.mule.sdk.api`, use only classes annotated with `@MinMuleVersion("4.4")`.
