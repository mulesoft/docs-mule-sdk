...
Use logging to troubleshoot SDK components.
SDK supports logging for the following components:
...
| <<connections#_pooling-strategy, Pooling Connection Provider>> | Logs information about connection management on pooling connection providers.
It provides connection pool status information as well as connection handling, indicating when connections are retrieved from the pool and returned back.
|===
...
In both cases, information is logged to the application's log file.
...
For example, add the following to the `log4j2.xml` file to enable verbose logging for Source:
