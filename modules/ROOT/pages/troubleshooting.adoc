...
Here are some examples that show which logs can be expected to be shown.

These logs were taken from the following simple application :

image:troubleshooting-app.png[sdk-troubleshooting-app]

This app consists on an FTP Connector listener which lists file in an FTP Server, and a logger that logs the name of the file it processes.
...
==== Polling Source item processing examples:
...
==== Polling Source watermarking example:
...
==== Connection Pools status examples:
