...
* Enable optional features that gracefully degrade on earlier Mule runtime versions
...
== When to Prevent Version Propagation

Consider using `@DoNotEnforceMinMuleVersion` to prevent version propagation when:
...
IMPORTANT: Do not use this annotation if you haven't verified that your component works correctly on earlier Mule runtime versions. If the component depends on the API being fully functional, do not suppress the version propagation.
