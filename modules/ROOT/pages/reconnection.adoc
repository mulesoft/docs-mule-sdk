...
When starting a Source, the execution of the `onStart()` method, if a connection problem is found and is required to be raised, it must be communicated through the SourceCallback, using the onConnectionException(), the Source will be stopped, and will try to reconnect and start the Source again.
