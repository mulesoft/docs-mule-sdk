...
A source is in the starting source stage when the onStart() method executes. If
a connection error exists, it must be communicated through the SourceCallback
using the onConnectionException() method. The source will stop and attempt to
reconnect.
