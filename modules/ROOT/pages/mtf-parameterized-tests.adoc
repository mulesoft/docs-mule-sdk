...
* The following section assumes that you are familiarized with the parameterization feature in MUnit. +
...
You can test different configs as follows:
...
If you want to execute the same operation but with different inputs, and each of them expecting different outputs, for example, testing an operation that replaces a string input with another:
...
Your test for several inputs by calling the same operation should look as follows:
