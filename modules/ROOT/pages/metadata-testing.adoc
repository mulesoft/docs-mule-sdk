...
NOTE: Metadata Testing is available since Mule Runtime version 4.2.0. When running your tests against a prior version,
metadata tests will be omitted
...
The `fromSchema` supports metadata types from `JSON schema`, `XSD schema` and `RAML Data Types`. The file extensions need to be `.json`, `.xsd` and `.raml` respectively.
