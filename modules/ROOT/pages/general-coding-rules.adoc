...
See xref:general::java-support.adoc[] to decide which Java version to use based on the target Mule runtime.

==== Java 11 Compatibility

Starting with Mule 4.2.0, JDK 11 is also supported. That means that when depending on native Java APIs, the module *must* account for the fact that the needed API might not be available given Java’s new modular architecture (also known as https://openjdk.org/projects/jigsaw/[Project Jigsaw], first introduced in Java 9).
...
For more information, refer to xref:java-version-support.adoc[].
