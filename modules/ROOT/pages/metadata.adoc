...
The purpose of using a MetadataKey parameter is to use the value given to it to resolve the metadata of output and inputs of the component where the key is defined. As such, you should not define a MetadataKey without any MetadataResolver for Inputs, Output, or Attributes. SDK versions 1.3 or above fail to compile connectors that contain a Component which has a MetadataKey parameter but does not have an Output Resolver or Input Resolvers.
